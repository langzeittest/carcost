...
//
function mod_carcost_common() {
    global $PHORUM;

    // Install once custom profiles field
    if (    !isset($PHORUM['mod_carcost_installed'])
         || !$PHORUM['mod_carcost_installed'] ) {
        include('./mods/carcost/install.php');
    }
}
...
//
function mod_carcost_tpl_cc_usersettings($profile) {
    if (isset($profile['USERPROFILE'])) {
        global $PHORUM;
        include(phorum_get_template('carcost::cc_usersettings'));
    }
    return $profile;
}
...
// Add sanity checks
//
function mod_carcost_sanity_checks($sanity_checks) {
    if (    isset($sanity_checks)
         && is_array($sanity_checks) ) {
        $sanity_checks[] = array(
            'function'    => 'mod_carcost_do_sanity_checks',
            'description' => 'CarCost Module'
        );
    }
...
function mod_carcost_do_sanity_checks() {
    ...
    if (!is_array($fields)) {
        ...
        if (!$field_exists) {
            return array(
                       PHORUM_SANITY_CRIT,
                       'The custom profile field for the CarCost '
    ...
    if ( !file_exists
             ("./mods/carcost/lang/{$PHORUM['language']}.php")
       ) {
        return array(
            PHORUM_SANITY_WARN,
            'Your default language is set to "'
                .htmlspecialchars($PHORUM['language'])
                .'", but the language file "mods/carcost/lang/'
                .htmlspecialchars($PHORUM['language'])
    ...
    foreach ($forums as $id => $forum) {
        if (    !empty($forum['language'])
             ...
             && !file_exists("./mods/carcost/lang/{$forum['language']}.php")
           ) {
            return array(
                PHORUM_SANITY_WARN,
    ...
    include('./mods/carcost/lang/english.php');
    $orig_data = $PHORUM['DATA']['LANG'];
    $orig_keys = array_keys($PHORUM['DATA']['LANG']['mod_carcost']);
    // Check all files in the module language directory
    $tmphandle = opendir('./mods/carcost/lang/');
    if ($tmphandle) {
        while ($file = readdir($tmphandle)) {
            if ($file == '.' || $file == '..' || $file == 'english.php')
                ...
                $PHORUM['DATA']['LANG'] = array();
                include("./mods/carcost/lang/{$file}");
                $new_keys = array_keys($PHORUM['DATA']['LANG']['mod_carcost']);
        ...
        return array(
                   ...
                   'Check if the mods/carcost/lang/ directory exists.'
               );
    }
